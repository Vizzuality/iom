    ...
    respond_to do |format|
      format.html do
        ...
        if @site.geographic_context_country_id
          ...
          sql="select c.id,count(ps.project_id) as count,c.name,r.center_lon as lon,
                      r.center_lat as lat,c.name,'/location/'||c.id as url,c.iso2_code as code,
                      (select count(*) from data_denormalization where regions_ids && ('{'||r.id||'}')::integer[] and is_active=true and site_id=#{@site.id}) as total_in_region
                from ((((((
                ...
                group by c.id,c.name,lon,lat,c.name,url,c.iso2_code,r.id"
        end
